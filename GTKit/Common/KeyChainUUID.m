...
static NSString * const aKeyChainUUIDIdentifier = @"com.getgaoding.Getios";
...
+ (NSString *)makeUUID
{
    ...
    NSString *BundleIdentifier = [NSString stringWithFormat:@"%lu%@%f", (unsigned long)(arc4random() % NSUIntegerMax),[infos objectForKey:@"CFBundleIdentifier"],now];
    
    const char *cStr = [BundleIdentifier UTF8String];
    unsigned char result[16];
    CC_MD5( cStr, (unsigned int)strlen(cStr), result );
    
    // MD5加密后再加上随机生成8个数字 确保标识的唯一 
